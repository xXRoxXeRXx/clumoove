...
func InitDB(connStr string) (*sql.DB, error) {
	...
	for attempt := 1; attempt <= 10; attempt++ {
		...
		if pingErr == nil {
			...
			); ALTER TABLE restore_previews ADD COLUMN IF NOT EXISTS config_fingerprint BYTEA; CREATE INDEX IF NOT EXISTS idx_restore_previews_claim ON restore_previews(created_at) WHERE status = 'QUEUED'; CREATE INDEX IF NOT EXISTS idx_restore_previews_owner ON restore_previews(user_id, created_at DESC);
