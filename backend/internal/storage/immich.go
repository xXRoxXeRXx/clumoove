...
func NewImmichProvider(baseURL, apiKey string) (*ImmichProvider, error) {
	...
	return &ImmichProvider{BaseURL: strings.TrimSuffix(u.String(), "/"), APIKey: apiKey, HTTPClient: &http.Client{Transport: tr, CheckRedirect: rejectEgressRedirect}, albums: make(map[string]string), albumIDs: make(map[string]string)}, nil
}
