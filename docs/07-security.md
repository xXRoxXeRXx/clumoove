...
File-manager routes accept only owned saved profile IDs. Item references and pagination cursors are AES-GCM sealed with dedicated domains and bind user ID, profile ID, resource type, item kind, name, and provider locator; cross-user, cross-profile, and tampered values are rejected. Delete, rename, copy, and move consume only those references. Mutations reject roots, invalid destination kinds, no-ops, and directory cycles; destinations must belong to the same profile. They report a collision before accepting an explicit `SKIP`, `OVERWRITE`, or `RENAME` retry. Native copies remain within the resolved provider profile and do not acquire a stream lease; approved streamed-copy fallbacks (Local, SMB, SFTP, FTP, and MEGA) acquire the per-user Redis lease and never retain data on disk. A move never deletes its source until copying has succeeded. Audit events record only provider, item kinds, conflict choice, execution mode, and outcome, never paths, names, IDs, or references. Download URLs hold only a cryptographically random, Redis-backed ticket with a 60-second TTL and atomic single use. Four download and four upload stream leases per user are enforced independently in Redis, fail closed on Redis errors, renew both their member and key TTL while streaming, and are released after streaming. Upload metadata is header-only, filename headers are Base64URL UTF-8, and the raw body must exactly match its declared length. Blob previews fetch the ticket rather than navigating to it; preview parser workers are terminated on close and generated Blob URLs are revoked. Paths, item names, opaque references, tickets, and credentials are excluded from normal logs and audit details.
