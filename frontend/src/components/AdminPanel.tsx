...
export function AdminPanel({ apiUrl, token, user, onBack }: AdminPanelProps) {
  ...
  return (
    <div className="max-w-5xl w-full mx-auto my-8 px-4 relative">
      <div className="relative glass-panel rounded-3xl p-7 shadow-portal hover:shadow-portal-hover border border-[var(--color-glass-border)] transition-all duration-500">
        ...
        <div className="flex items-center justify-between mb-6">
          <button
            onClick={onBack}
            className="flex items-center gap-2 text-xs font-mono text-[var(--color-text-muted)] hover:text-portal-orange transition-colors cursor-pointer"
          >
        ...
        <div className="flex flex-wrap gap-2 mb-6 border-b border-[var(--color-border)]">
          {([
            ...
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`flex items-center gap-2 px-3.5 py-2 text-xs font-semibold rounded-t-lg transition-colors cursor-pointer ${
                tab === key
                  ? 'text-portal-orange border-b-2 border-portal-orange -mb-px'
        ...
        {message && <div className="mb-4"><MessageBanner message={message} /></div>}
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          className="px-3 py-1.5 text-xs bg-[var(--color-bg-secondary)]/50 border border-[var(--color-border)] rounded-lg focus:outline-none focus:ring-2 focus:ring-portal-orange/30"
        />
        <select value={roleFilter} onChange={(e) => { setRoleFilter(e.target.value); setPage(1); }}
          className="px-3 py-1.5 text-xs bg-[var(--color-bg-secondary)]/50 border border-[var(--color-border)] rounded-lg">
        ...
        <button onClick={() => setShowCreate((v) => !v)}
          className="ml-auto flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-gradient-to-r from-portal-orange to-orange-500 text-white rounded-lg cursor-pointer">
          <UserPlus className="w-4 h-4" /> {t('admin.users.create')}
        </button>
      </div>

      {showCreate && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-tertiary)]/40">
          ...
          <div className="md:col-span-2 flex justify-end gap-2">
            <button onClick={() => setShowCreate(false)} className="px-3 py-1.5 text-xs rounded-lg border border-[var(--color-border)] cursor-pointer">{t('common.cancel')}</button>
            <button onClick={create} className="px-3 py-1.5 text-xs font-bold bg-gradient-to-r from-portal-orange to-orange-500 text-white rounded-lg cursor-pointer">{t('common.save')}</button>
          </div>
        </div>
      )}
                  ...
                  <div className="flex justify-end gap-1.5">
                    {u.active ? (
                      <button title={t('admin.users.suspend')} onClick={() => act(() => adminApi.suspendUser(apiUrl, token, u.id!), 'admin.users.suspendedOk')}
                        className="p-1.5 rounded-lg border border-[var(--color-border)] text-rose-600 hover:bg-rose-50/50 cursor-pointer"><Ban className="w-3.5 h-3.5" /></button>
                    ) : (
                      <button title={t('admin.users.reactivate')} onClick={() => act(() => adminApi.reactivateUser(apiUrl, token, u.id!), 'admin.users.reactivatedOk')}
                        className="p-1.5 rounded-lg border border-[var(--color-border)] text-emerald-600 hover:bg-emerald-50/50 cursor-pointer"><CheckCircle2 className="w-3.5 h-3.5" /></button>
                    )}
                    <button title={t('admin.users.toggleRole')} onClick={() => act(() => adminApi.updateRole(apiUrl, token, u.id!, u.role === 'ADMIN' ? 'USER' : 'ADMIN'), 'admin.users.roleChanged')}
                      className="p-1.5 rounded-lg border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-tertiary)] cursor-pointer">
                    ...
                    {u.id !== currentUserID && (
                      <button title={t('admin.users.delete')} onClick={() => { if (confirm(t('admin.users.deleteConfirm'))) act(() => adminApi.deleteUser(apiUrl, token, u.id!), 'admin.users.deletedOk'); }}
                        className="p-1.5 rounded-lg border border-[var(--color-border)] text-rose-600 hover:bg-rose-50/50 cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></button>
                    )}
                  </div>
                </td>
              </tr>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input type="date" value={to} onChange={(e) => { setTo(e.target.value); setPage(1); }}
          className="px-3 py-1.5 text-xs border border-[var(--color-border)] rounded-lg" />
        <button onClick={() => { setPage(1); }} className="p-1.5 rounded-lg border border-[var(--color-border)] text-[var(--color-text-secondary)] cursor-pointer"><RefreshCw className="w-3.5 h-3.5" /></button>
      </div>
  ...
  return (
    <div className="flex items-center justify-between text-xs">
      <button
        ...
        className="px-3 py-1.5 rounded-lg border border-[var(--color-border)] disabled:opacity-40 cursor-pointer"
      >
        ←
      </button>
