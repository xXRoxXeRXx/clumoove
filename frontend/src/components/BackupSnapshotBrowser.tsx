...
export function BackupSnapshotBrowser({ apiUrl, token, jobID, onBack }: Props) {
  ...
  const verifyModeId = useId();
  const verifyBudgetId = useId();
  const verifyFullId = useId();
  const targetProfileSelectId = useId();
  const directProviderSelectId = useId();
  const directUrlInputId = useId();
  const directUsernameInputId = useId();
  const directPasswordInputId = useId();
  const directRefreshTokenInputId = useId();
  const targetFolderInputId = useId();
  const conflictStrategySelectId = useId();
  const restoreThreadsInputId = useId();
  const restoreBandwidthInputId = useId();
       ...
       <div className="mb-4 space-y-2 border border-[var(--color-border)] p-3" aria-label={t('backup.verifyTitle')}>
         <h3 className="text-sm font-semibold">{t('backup.verifyTitle')}</h3>
          <div className="flex flex-wrap items-end gap-2 text-xs">
            <div>
              <label htmlFor={verifyModeId} className="block">{t('backup.verifyMode')}</label>
              <select id={verifyModeId} className="ui-input mt-1" value={verifyMode} onChange={(event) => setVerifyMode(event.target.value)}>
                <option value="METADATA">{t('backup.verifyMetadata')}</option>
                <option value="BUDGETED">{t('backup.verifyBudgeted')}</option>
                <option value="FULL">{t('backup.verifyFull')}</option>
              </select>
            </div>
            {verifyMode === 'BUDGETED' && (
              <div>
                <label htmlFor={verifyBudgetId} className="block">{t('backup.verifyBudget')}</label>
                <input id={verifyBudgetId} className="ui-input mt-1 w-24" type="number" min="64" max="1048576" value={verifyBudgetMiB} onChange={(event) => setVerifyBudgetMiB(Number(event.target.value))} />
              </div>
            )}
            {verifyMode === 'FULL' && (
              <div className="flex items-center gap-1">
                <input id={verifyFullId} type="checkbox" checked={verifyFullConfirmed} onChange={(event) => setVerifyFullConfirmed(event.target.checked)} />
                <label htmlFor={verifyFullId}>{t('backup.verifyConfirmFull')}</label>
              </div>
            )}
  ...
  return <section className="ui-card p-5" aria-label={t('backup.snapshotContents')}>
    ...
    {restoreOpen && <div className="fixed inset-0 z-[var(--layer-dialog)] flex items-center justify-center bg-[var(--color-overlay)] p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setRestoreOpen(false); }}>
      ...
      <fieldset className="space-y-2 border border-[var(--color-border)] p-3"><legend className="px-1 text-xs font-medium">{t('backup.restoreTarget')}</legend><label className="mr-3 text-xs"><input type="radio" checked={targetMode === 'profile'} onChange={() => setTargetMode('profile')} /> {t('backup.restoreSavedTarget')}</label><label className="text-xs"><input type="radio" checked={targetMode === 'direct'} onChange={() => setTargetMode('direct')} /> {t('backup.restoreDirectTarget')}</label>{targetMode === 'profile' ? <div className="mt-2"><label htmlFor={targetProfileSelectId} className="block text-xs font-medium">{t('backup.restoreChooseTarget')}</label><select id={targetProfileSelectId} className="ui-input mt-1 w-full" value={targetProfileID} onChange={(event) => setTargetProfileID(event.target.value)}><option value="">{t('backup.restoreChooseTarget')}</option>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name} ({profile.provider})</option>)}</select></div> : <div className="mt-2 grid gap-2"><div><label htmlFor={directProviderSelectId} className="text-xs">{t('backup.restoreProvider')}</label><select id={directProviderSelectId} className="ui-input mt-1 w-full" value={directProvider} onChange={(event) => setDirectProvider(event.target.value)}>{['nextcloud', 'opencloud', 'webdav', 'dropbox', 'google', 'onedrive', 'hidrive', 's3', 'smb', 'sftp', 'ftp', 'magentacloud', 'koofr', 'seafile', 'mega', 'local'].map((provider) => <option key={provider} value={provider}>{provider}</option>)}</select></div>{directProvider !== 'local' && <><div><label htmlFor={directUrlInputId} className="text-xs">{t('backup.restoreEndpoint')}</label><input id={directUrlInputId} className="ui-input mt-1 w-full" value={directURL} onChange={(event) => setDirectURL(event.target.value)} autoComplete="url" /></div><div><label htmlFor={directUsernameInputId} className="text-xs">{t('backup.restoreUsername')}</label><input id={directUsernameInputId} className="ui-input mt-1 w-full" value={directUsername} onChange={(event) => setDirectUsername(event.target.value)} autoComplete="username" /></div><div><label htmlFor={directPasswordInputId} className="text-xs">{t('backup.restorePassword')}</label><input id={directPasswordInputId} className="ui-input mt-1 w-full" type="password" value={directPassword} onChange={(event) => setDirectPassword(event.target.value)} autoComplete="current-password" /></div>{['dropbox', 'google', 'onedrive', 'hidrive'].includes(directProvider) && <div><label htmlFor={directRefreshTokenInputId} className="text-xs">{t('backup.restoreRefreshToken')}</label><input id={directRefreshTokenInputId} className="ui-input mt-1 w-full" type="password" value={directRefreshToken} onChange={(event) => setDirectRefreshToken(event.target.value)} autoComplete="off" /></div>}</>}</div>}</fieldset>
       <div><label htmlFor={targetFolderInputId} className="block text-xs font-medium">{t('backup.restoreFolder')}</label><input id={targetFolderInputId} className="ui-input mt-1 w-full" value={targetRoot} onChange={(event) => setTargetRoot(event.target.value)} /></div>
       <div><label htmlFor={conflictStrategySelectId} className="block text-xs font-medium">{t('backup.restoreConflict')}</label><select id={conflictStrategySelectId} className="ui-input mt-1 w-full" value={strategy} onChange={(event) => setStrategy(event.target.value)}><option value="RENAME">{t('backup.restoreRename')}</option><option value="SKIP">{t('backup.restoreSkip')}</option><option value="OVERWRITE">{t('backup.restoreOverwrite')}</option></select></div>
       <div className="grid gap-3 sm:grid-cols-2"><div><label htmlFor={restoreThreadsInputId} className="block text-xs font-medium">{t('backup.restoreThreads')}</label><input id={restoreThreadsInputId} className="ui-input mt-1 w-full" type="number" min="1" max="16" value={restoreThreads} onChange={(event) => setRestoreThreads(Math.max(1, Math.min(16, Number(event.target.value) || 1)))} /></div><div><label htmlFor={restoreBandwidthInputId} className="block text-xs font-medium">{t('backup.restoreBandwidth')}</label><input id={restoreBandwidthInputId} className="ui-input mt-1 w-full" type="number" min="0" max="1000" value={restoreBandwidthMbps} onChange={(event) => setRestoreBandwidthMbps(Math.max(0, Math.min(1000, Number(event.target.value) || 0)))} /></div></div>
