...
- **Rate Limiting & Lockouts**: A Redis-backed fixed-window limiter keyed by endpoint group and client IP (honours `X-Forwarded-For` only behind a trusted proxy — see `TRUSTED_PROXY`) provides consistent multi-instance limits without cross-throttling endpoint groups. Limits: Login 10/min, Register 5/5min, Connect/Browse/Mkdir 30/min, TOTP 10/min, migration stream (SSE) 60/min with max 10 concurrent streams per user. 5 failed logins → 15-min lockout; 5 failed TOTP attempts → 15-min lockout (both atomic, in `db`).
