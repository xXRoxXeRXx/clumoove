...
- **Data integrity (3-way hash check)**: An `io.TeeReader` computes the source-side hash while streaming (`SHA1` default; `MD5`/`SHA256`/`DROPBOX` per provider). For a HiDrive target it also computes the native hierarchical `HIDRIVE` `chash`, which is compared with the server-side target `chash`; HiDrive source-to-target compares native hashes directly. The provider decodes URL-escaped API response paths before indexing to prevent double-escaped 404s. Where hashes cannot be compared, fall back to size comparison to avoid false "corrupted" verdicts.
