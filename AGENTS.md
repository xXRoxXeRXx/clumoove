...
- **Queue**: PostgreSQL-native via `SELECT … FOR UPDATE SKIP LOCKED` in `queue.DequeueSQL()`: migrations dequeue in `RUNNING`/`INDEXING`; sync jobs dequeue only in `RUNNING` (their `INDEXING` phase builds a fresh delta). Each claimed sync pass increments `run_generation`; tasks persist that value as `pass_generation`, and lifecycle/progress updates require the matching generation. On cancellation, the coordinator retains its cross-instance pass lock while workers drain, with a 30-second bounded grace period. Redis is used **only** for worker-liveness heartbeats and distributed recovery locks (`SET NX`).
...
- **Transfer & conflict resolution**: Files use `SKIP` (with size-match short-circuit), `OVERWRITE` (upload to `.tmp` then atomic rename), or `RENAME` (up to 100 suffix attempts). Filename sanitization (`internal/sanitize`) and case-collision resolution run first for case-insensitive targets. Immich targets are the exception: they require native duplicate `SKIP` and do not support overwrite, rename, delete, or atomic rename. Calendars/contacts are always overwritten (a `SKIP` would leave stale entries). Transfers are streamed through a RAM buffer (zero disk retention); files > 50 MB use chunked upload.
...
- **Rate Limiting & Lockouts**: A Redis-backed fixed-window limiter keyed by endpoint group and client IP (honours `X-Forwarded-For` only behind a trusted proxy — see `TRUSTED_PROXY`) provides consistent multi-instance limits without cross-throttling endpoint groups. Limits: Login 10/min, Register 5/5min, Connect/Browse/Mkdir 30/min, Migration/sync create or start 10/min, TOTP 10/min, migration stream (SSE) 60/min with max 10 concurrent streams per user. 5 failed logins → 15-min lockout; 5 failed TOTP attempts → 15-min lockout (both atomic, in `db`).
